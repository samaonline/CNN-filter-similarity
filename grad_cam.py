...
class _BaseWrapper(object):
    def __init__(self, model, layer):
        ...
        if False:
            ...
            self.model = nn.Sequential(*A[:layer])
        else:
            A = list(model.modules()) 
            self.model = nn.Sequential(*A[2:2+layer])
    ...
    def _encode_one_hotG(self, myshape, ids):
        one_hot = torch.zeros_like(myshape).to(self.device)
        one_hot[:,ids,...] = 1.0
        return one_hot
    
    def forward(self, image):

        self.featA = self.model(image)
    ...
    def backward(self, ids):
        ...
        one_hot = self._encode_one_hotG(self.featA, ids)
        
        self.model.zero_grad()
        self.featA.backward(gradient=one_hot, retain_graph=True)
...
class GuidedBackPropagation(BackPropagation):
    ...
    def __init__(self, model, layer):
        super(GuidedBackPropagation, self).__init__(model, layer)
